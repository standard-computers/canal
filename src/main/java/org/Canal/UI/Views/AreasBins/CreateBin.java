...
public class CreateBin extends JInternalFrame {
    ...
    public CreateBin(DesktopState desktop, Location location) {
        super("New Location Bin", false, true, false, true);
        setFrameIcon(new ImageIcon(CreateBin.class.getResource("/icons/bins.png")));
        String generatedId;
        JTextField areaIdField = Elements.input();
        Selectable locationIdField = Selectables.allLocations();
        if(location == null){
            generatedId = "BN-" + Engine.getAreas().size();
        }else{
            generatedId = "BN-" + (Engine.getAreas(location.getId()).size() + 1) + "-" + location.getId();
