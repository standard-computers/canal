...
public class CreateOutboundDeliveryOrder extends LockeState {
    ...
    private JPanel toolbar() {
        ...
        IconButton create = new IconButton("Create", "create", "Create Outbound Delivery (Ctrl + S)");
        create.addActionListener(_ -> {
            ...
            if (!truckId.getText().trim().isEmpty()) {
                Truck outboundTruck = Engine.getTruck(truckId.getText().trim());
                if (outboundTruck == null) {
                    JOptionPane.showMessageDialog(null, "Selected truck does not exist!", "Truck ID", JOptionPane.ERROR_MESSAGE);
                    return;
                }
            }

            Delivery newOutboundDelivery = new Delivery();
            newOutboundDelivery.setType("TRANS/ODO");
            newOutboundDelivery.setId(Engine.generateId("TRANS/ODO"));
            newOutboundDelivery.setSalesOrder(salesOrderId.getText().trim());
            newOutboundDelivery.setPurchaseOrder(purchaseOrderId.getText().trim());
            newOutboundDelivery.setOrigin(origin.getText().trim());
            newOutboundDelivery.setDestination(destination.getText().trim());
            newOutboundDelivery.setDestinationArea(destinationArea.getText().trim());
            newOutboundDelivery.setDestinationDoor(destinationBin.getText().trim());
            newOutboundDelivery.setTotal(deliveryValue.getText().trim());
            newOutboundDelivery.setTruck(truckId.getText().trim());
        ...
        toolbar.add(Elements.header("Create an Outbound Delivery", SwingConstants.LEFT), BorderLayout.CENTER);
